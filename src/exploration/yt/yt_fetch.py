...
def yt_fetch(num_vids=1e6):
  ...
  date_range_end = None
  file_path = 'data/yt_stages/raw/yt_data4.csv'
    ...
    for vid_i, vid in enumerate(videos):
      ...
      for com_page_i in range(100):
        ...
        for com in com_res['items']:
          try:
            if 'publishedAt' not in com['snippet']['topLevelComment']['snippet']:
              print(com)
              continue
...
def remove_duplicates():
  ...
  for file in os.listdir('../../../data/yt_stages/raw'):
    file_df = pd.read_csv(f'../../../data/yt_stages/raw/{file}')
    file_df = file_df[file_df['date'].notna()]
    dfs.append(file_df)
  df = pd.concat(dfs)
